"""
================================================================================
                FINAL SPDC SIMULATION WITH ENTANGLEMENT DETECTION
================================================================================
"""
...
print("="*80)
print("FINAL SPDC SIMULATION - PERES-HORODECKI ENTANGLEMENT TEST")
print("="*80)
print(f"QuTiP version: {qutip.__version__}")
print("="*80)

# ============================================================================
# PARAMETERS (ADJUST FOR YOUR SETUP)
# ============================================================================

N = 5                     # Photon number truncation
tlist = np.linspace(0, 25, 100)
g = 0.12                  # Coupling strength
gamma = 0.003             # Loss rate
alpha = 2.0               # Coherent pump amplitude
...
# Reduce to signal-idler subspace (trace out pump)
rho_si = ptrace(rho_final, [1, 2])
rho_si.dims = [[N, N], [N, N]]   # set correct dimensions

# Partial transpose on the second subsystem (idler)
rho_pt = partial_transpose(rho_si, [1])   # transpose idler

# Compute eigenvalues of the partially transposed density matrix
eigvals = rho_pt.eigenenergies()
neg_evals = eigvals[eigvals < -1e-8]
is_entangled = len(neg_evals) > 0
...
print(f"\nPartial transpose eigenvalues: {eigvals[:5]} ...")
...
if is_entangled:
    print("  → State is ENTANGLED (negative partial transpose)")
else:
    print("  → State appears SEPARABLE (no negative eigenvalues)")
...
axes[1, 1].text(0.5, 0.5, f'Negativity: {negativity:.4f}\nLog Neg: {log_neg:.4f}\nEntropy: {entropy_si:.4f} nats\nPurity: {purity_si:.4f}',
